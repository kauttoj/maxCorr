...
for i = 1:N
    ...
    cfg.filenames(i).cfgfile = [filepath,filesep,sprintf('cfg_subject%i.mat',i)];    
...
save(sprintf('maxCorr_group_cfg_%s.mat',datetime('now','Format','defaultdate')),'cfg','-v7.3');
...
for i = 1:N
    ...
    if cfg.recompute_all || ~exist(cfg.filenames(i).covfile,'file')
       jobnames{i} = maxCorr_sendjob(cfg.filenames(i).jobfile1,cfg.filenames(i).cfgfile,cfg.maxCorr_path,'maxCorr_compute_covariance');
    else
       wait_for(i)=false;
...
wait_for_jobs(cfgfiles(wait_for),jobnames(wait_for),jobfiles(wait_for),stage);
...
for i = 1:N,
    ...
    if cfg.recompute_all || ~exist(cfg.filenames(i).targetfile,'file')
       jobnames{i} = maxCorr_sendjob(cfg.filenames(i).jobfile1,cfg.filenames(i).cfgfile,cfg.maxCorr_path,'maxCorr_clean_dataset');
    else
       wait_for(i)=false;
       save(cfg.filenames(i).cfgfile,'cfg','stage','-v7.3'); 
    end
