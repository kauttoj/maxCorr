function jobname = maxCorr_sendjob(filename,paramfile,codepath,funfile)

logfile = [filename,'_log'];

dlmwrite(filename, '#!/bin/sh', '');
dlmwrite(filename, '#SBATCH -p batch','-append','delimiter','');
dlmwrite(filename, '#SBATCH -t 00:40:00','-append','delimiter','');
...
dlmwrite(filename, '#SBATCH --mem=15000','-append','delimiter','');
dlmwrite(filename, 'module load matlab','-append','delimiter','');
dlmwrite(filename,sprintf('srun matlab -nosplash -nodisplay -nodesktop -r "cd(''%s'');sprintf(''current path: %%s'',pwd());%s(''%s'');exit;"',codepath,funfile,paramfile),'-append','delimiter','');
